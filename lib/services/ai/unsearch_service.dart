...
class UnSearchService {
  ...
  Future<List<UnSearchResult>> searchResources({
    ...
    try {
      ...
      final response = await _client.post(
        Uri.parse('${AppConstants.unsearchApiUrl}/search'),
        headers: {
        ...
        body: jsonEncode({
          ...
          'type': 'auto',
          'category': 'learning',
          'subcategory': resourceTypes ??
              ['course', 'tutorial', 'video', 'book', 'documentation'],
          'text': true,
          'highlights': true,
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('UnSearch API error: ${error['error']}');
      }
    } catch (e) {
  ...
  List<UnSearchResult> _parseResults(Map<String, dynamic> data) {
    final results = <UnSearchResult>[];

    if (data['results'] != null) {
      for (final item in data['results']) {
        results.add(UnSearchResult(
          id: item['id'] ?? '',
...
class UnSearchResult {
  ...
  factory UnSearchResult.fromJson(Map<String, dynamic> json) => UnSearchResult(
        id: json['id'] ?? '',
